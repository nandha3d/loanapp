...
import {
  cleanupZoloFundUiScenario,
  seedZoloFundUiScenario,
  type ZoloFundUiSeed,
} from '../tests/e2e/helpers/zolofundSeed';
import { loginAs, modulePath } from '../tests/e2e/helpers/zolofundSelectors';
...
test.describe('cross-browser login compatibility', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    seed = await seedZoloFundUiScenario(`auth-${project}-${Date.now()}`);
  });

  test.afterAll(async () => {
    await cleanupZoloFundUiScenario(seed.runId);
  });
