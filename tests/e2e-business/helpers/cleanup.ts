...
export async function cleanupRunData(runId = getRunId()) {
  ...
  const deletions = [
    ...
    () => prisma.chitDocument.deleteMany({ where: { tenantId: { in: tenantIds } } }),
    () => prisma.chitReceipt.deleteMany({ where: { tenantId: { in: tenantIds } } }),
    () => prisma.chitSecurity.deleteMany({ where: { tenantId: { in: tenantIds } } }),
    () => prisma.chitPenalty.deleteMany({ where: { tenantId: { in: tenantIds } } }),
    () => prisma.chitAuctionAttendance.deleteMany({ where: { tenantId: { in: tenantIds } } }),
    () => prisma.chitBid.deleteMany({ where: { tenantId: { in: tenantIds } } }),
