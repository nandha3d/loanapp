...
export default defineConfig({
  ...
  reporter: [
    ['list'],
    ['json', { outputFile: path.join(ROOT, 'test-report', 'goldloan-results.json') }],
    ['html', { outputFolder: path.join(ROOT, 'test-report', 'playwright-html-gold'), open: 'never' }],
  ],
  outputDir: path.join(ROOT, 'test-results', 'goldloan'),
  use: {
    baseURL: BASE_URL,
  ...
  projects: [{ name: 'goldloan', use: { ...devices['Desktop Chrome'] } }],
});
