# ZoloFund V5 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the ZoloFund v5 bug-fix prompt: complete the headless REST API layer, fix broken navigation, remove duplicated hardcoded configuration, improve theme/mobile behavior, repair credit-score logic, and add fixed/percentage deduction support with reusable loan templates.
...
- [ ] Ignore `scratch/`, `database/*.sql`, and `zolofund_export*.sql`.
