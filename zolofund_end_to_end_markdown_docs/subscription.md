...
Also replace hardcoded/default-tenant behavior so non-developer users always resolve to their own `tenantId`, not the seeded `zolofund/default` tenant.
...
- Auth/security tests proving a superadmin resolves to their own tenant instead of `default/zolofund`.
