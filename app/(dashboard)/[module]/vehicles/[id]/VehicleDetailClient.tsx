          ...
          <table style={{ width: '100%' }}>
            <tbody>
              <tr>
                ...
                <td><Link href={`/customers/${vehicle.customer.customerCode}`}>{vehicle.customer.name}</Link> <span style={{ fontSize: '.75rem', color: 'var(--text-light)' }}>({vehicle.customer.customerCode})</span></td>
              </tr>
              <tr>
                <td style={{ color: 'var(--text-secondary)' }}>{d.loan}</td>
                <td>{vehicle.loan ? <Link href={`/loans/${vehicle.loan.loanCode}`}>{vehicle.loan.loanCode}</Link> : '—'}</td>
              </tr>
            </tbody>
          </table>
        </div>
