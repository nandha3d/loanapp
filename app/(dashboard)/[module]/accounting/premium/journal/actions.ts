...
export async function listJournalEntries(filter?: { from?: string; to?: string; status?: string; search?: string; page?: number }) {
  ...
  const where: any = {
    tenantId, appType, entryDate: { gte: from, lte: to },
...
export async function postEntry(input: { entryDate: string; narration?: string; branchId?: string; lines: Array<{ accountId: string; debit: number; credit: number; description?: string; lineNo: number }> }) {
  ...
  const appType = await getUserAppType();
  const branchId = await getActiveBranchId();
  if (input.branchId && input.branchId !== branchId) return { error: 'branch_mismatch' };
  if (!branchId) return { error: 'branch_mismatch' };
  const accountIds = [...new Set(input.lines.map((line) => line.accountId))];
  if (await prisma.account.count({ where: { tenantId, id: { in: accountIds } } }) !== accountIds.length) return { error: 'not_found' };
  ...
  if (totalDr > cap) {
    const entry = await prisma.$transaction(async (tx) => {
      const created = await tx.journalEntry.create({ data: { tenantId, appType, branchId, entryDate, narration: input.narration, status: 'pending_approval', sourceType: 'manual', totalDebit: totalDr, totalCredit: totalCr, createdById: userId, lines: { create: input.lines.map((l) => ({ accountId: l.accountId, debit: l.debit, credit: l.credit, description: l.description, lineNo: l.lineNo })) } } });
      await tx.accountingApproval.create({ data: { tenantId, appType, branchId, entityType: 'journal_entry', entityId: created.id, amount: totalDr, level: 1, approverRole: 'superadmin', requestedById: userId } });
      return created;
    });
  ...
  const entry = await prisma.$transaction(async (tx) => {
    const je = await tx.journalEntry.create({ data: { tenantId, appType, branchId: branchId ?? null, entryNo, entryDate, narration: input.narration, status: 'posted', sourceType: 'manual', totalDebit: totalDr, totalCredit: totalCr, createdById: userId, approvedById: userId, approvedAt: new Date(), lines: { create: input.lines.map(l => ({ accountId: l.accountId, debit: l.debit, credit: l.credit, description: l.description, lineNo: l.lineNo })) } }, include: { lines: true } });
    for (const line of je.lines) await bumpAccountBalance(tx as any, line.accountId, entryDate, line.debit, line.credit);
    await tx.accountingAuditLog.create({ data: { tenantId, appType, branchId, userId, action: 'post', entityType: 'journal_entry', entityId: je.id, after: JSON.stringify({ entryNo, totalDebit: totalDr }) } });
    return je;
  });
  revalidatePath('/accounting/premium/journal');
...
export async function saveDraftEntry(input: { entryDate: string; narration?: string; lines: Array<{ accountId: string; debit: number; credit: number; description?: string; lineNo: number }> }) {
  ...
  const totalDr = input.lines.reduce((s, l) => s + (l.debit || 0), 0);
  const totalCr = input.lines.reduce((s, l) => s + (l.credit || 0), 0);
  const entry = await prisma.journalEntry.create({ data: { tenantId, appType, branchId, entryDate, narration: input.narration, status: 'draft', sourceType: 'manual', totalDebit: totalDr, totalCredit: totalCr, createdById: userId, lines: { create: input.lines.map((l, i) => ({ accountId: l.accountId, debit: l.debit, credit: l.credit, description: l.description, lineNo: i })) } } });
  revalidatePath('/accounting/premium/journal');
...
export async function postDraftEntry(id: string) {
  ...
  if (totalDr > cap) {
    await prisma.$transaction([
      prisma.journalEntry.update({ where: { id, tenantId, appType, status: 'draft' }, data: { status: 'pending_approval' } }),
      prisma.accountingApproval.create({ data: { tenantId, appType, branchId: draft.branchId, entityType: 'journal_entry', entityId: id, amount: totalDr, level: 1, approverRole: 'superadmin', requestedById: userId } }),
    ]);
    revalidatePath('/accounting/premium/journal');
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.accountingAuditLog.create({ data: { tenantId, appType, branchId: draft.branchId, userId, action: 'post', entityType: 'journal_entry', entityId: id, after: JSON.stringify({ entryNo, totalDebit: totalDr }) } });
  });
  revalidatePath('/accounting/premium/journal');
...
export async function deleteDraftEntry(id: string) {
  ...
  const appType = await getUserAppType();
  const branchId = await getActiveBranchId();
  const draft = await prisma.journalEntry.findFirst({ where: { id, tenantId, appType, ...(branchId ? { branchId } : {}), status: 'draft' } });
  if (!draft) return { error: 'not_found' };
...
export async function reverseEntry(id: string, reason: string) {
  ...
  const appType = await getUserAppType();
  const branchId = await getActiveBranchId();
  const original = await prisma.journalEntry.findFirst({ where: { id, tenantId, appType, ...(branchId ? { branchId } : {}), status: 'posted' }, include: { lines: true } });
  ...
  const reversal = await prisma.$transaction(async (tx) => {
    ...
    await tx.accountingAuditLog.create({ data: { tenantId, appType, branchId: original.branchId, userId, action: 'reverse', entityType: 'journal_entry', entityId: id, after: JSON.stringify({ reversalId: rev.id, reason }) } });
    return rev;
  });
  revalidatePath('/accounting/premium/journal');
...
export async function approveEntry(id: string) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.accountingApproval.updateMany({ where: { tenantId, appType, entityId: id, status: 'pending' }, data: { status: 'approved', approvedById: userId, reviewedAt: new Date() } });
    await tx.accountingAuditLog.create({ data: { tenantId, appType, branchId: entry.branchId, userId, action: 'approve', entityType: 'journal_entry', entityId: id, after: JSON.stringify({ entryNo }) } });
  });
  revalidatePath('/accounting/premium/journal');
  return { success: true };
}
...
export async function rejectEntry(id: string, note: string) {
  ...
  const appType = await getUserAppType();
  const branchId = await getActiveBranchId();
  const entry = await prisma.journalEntry.findFirst({ where: { id, tenantId, appType, ...(branchId ? { branchId } : {}) }, select: { id: true } });
  if (!entry) return { error: 'not_found' };
  await prisma.journalEntry.update({ where: { id }, data: { status: 'rejected' } });
  await prisma.accountingApproval.updateMany({ where: { tenantId, appType, entityId: id, status: 'pending' }, data: { status: 'rejected', approvedById: userId, reviewNote: note, reviewedAt: new Date() } });
