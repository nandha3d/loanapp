...
- **SCOPE-4** — Unbranched records (`branchId: null`) are visible to superadmins/developers only. That is the safe failure direction. Orphans are a **data** problem, repaired by backfill scripts — never by widening a `where` clause.
...
- **SCOPE-7** — When a creator has no branch (e.g. a superadmin), inherit the **customer's** `branchId` so the new record is never branchless. See the loan create in `app/api/v1/loans/route.ts`.
