...
A tenant runs one or more **modules** (verticals). All six share the same database schema models (customer/loan/collection/accounting tables), but **data rows are strictly isolated per module (`appType`)**: a customer belongs to exactly one module and is never shared across modules (see SCOPE-17).
...
- **SCOPE-17** — **Customers are strictly module-scoped (`appType`) and MUST NOT be shared across modules.** Each vertical (`microlending`, `autofinance`, `chitfunds`, `goldloan`, `property`, `productfinance`) maintains its own isolated customer base. A customer registered under one module does not exist in, cannot be loaned to, cannot be picked in, and cannot be enrolled in another module. Customer creation checks uniqueness per `(tenantId, appType, phone)`. Any attempt to cross-link a customer from one `appType` into a loan, vehicle, chit group or pledge of another `appType` MUST be rejected with `404 Not Found` or `400 Bad Request`. All pickers, search inputs, and lookup endpoints MUST filter `Customer` by `{ tenantId, appType }`.
