...
-- float per module.
--
-- Ordering matters: add columns, then CREATE the new (tenant_id, app_type, ...)
-- indexes, and only THEN drop the old ones. The old account_entries /
-- agent_accounts / branch_cash_accounts indexes back foreign keys on tenant_id;
-- the new indexes are left-prefixed by tenant_id so they keep the FK covered,
-- which lets the old indexes drop without "needed in a foreign key constraint".
...
-- CreateIndex (new — created before dropping the old so FK stays covered)
...
-- DropIndex (old — safe now that the new tenant_id-prefixed indexes cover the FKs)
