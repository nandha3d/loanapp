  ...
  <BODY>
    <IMPORTDATA>
      <REQUESTDESC>
        ...
        <STATICVARIABLES>
          <SVCURRENTCOMPANY>LoanTrack Books</SVCURRENTCOMPANY>
        </STATICVARIABLES>
      </REQUESTDESC>
      <REQUESTDATA>
        <TALLYMESSAGE>
...
- LoanTrack `Account.classType + subType` → Tally Group.
...
- LoanTrack JE → Tally Journal voucher.
- LoanTrack Bill → Tally Purchase voucher (`VCHTYPE="Purchase"`).
- LoanTrack Payment → Tally Payment voucher.
...
**Caveats:** Tally must be running on a machine reachable from the LoanTrack server. For most tenants this is a local-network setup; we surface a warning that the connector is **not** for cloud deployments.
...
| EXP-05 | Tally connector test on valid URL | `{ok: true, company: 'LoanTrack Books'}` |
