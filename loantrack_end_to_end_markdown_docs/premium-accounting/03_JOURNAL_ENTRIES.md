...
The premium module derives JEs from existing LoanTrack events. The derivation is done by a worker queue: every CollectionEntry / Loan create / Penalty insert fires a Prisma `$on('beforeExit')` middleware (or, simpler, an `after-insert` hook in the relevant server action) that calls `lib/accounting/postings.ts → enqueuePostingJob`. A background runner (Vercel cron every 1 min) processes the queue.
