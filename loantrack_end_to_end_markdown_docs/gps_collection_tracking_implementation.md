...
**For:** LoanTrack Micro Lending — Admin, Superadmin, Field Agent Roles  
...
async function geocodeWithNominatim(
  ...
  const response = await fetch(
    ...
    {
      headers: {
        'User-Agent': 'LoanTrack/1.0 (contact@yourdomain.com)',
      }
    }
  )
...
export default function RouteMap({ agents, selectedAgentId }: RouteMapProps) {
  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      map.eachLayer(layer => {
        if ((layer as any)._loantrackLayer) map.removeLayer(layer)
      })
      ...
      for (const agent of agentsToShow) {
        ...
        if (agent.path.length > 1) {
          ...
          ;(polyline as any)._loantrackLayer = true
          polyline.addTo(map)
        }
        ...
        if (agent.lastLocation) {
          ...
          ;(marker as any)._loantrackLayer = true
...
*Document version 1.0 — LoanTrack GPS Collection Tracking Module*  
