...
**For:** LoanTrack Micro Lending — Licensed NBFCs & MFIs  
...
0 2 * * * curl -s -X POST \
  ...
  >> /var/log/loantrack/npa-cron.log 2>&1

# Penalty accrual (existing) at 1:00 AM
0 1 * * * curl -s -X POST \
  -H "x-cron-secret: your_cron_secret_here" \
  https://yourdomain.com/api/cron/accrue-penalties \
  >> /var/log/loantrack/penalty-cron.log 2>&1
```

```bash
# Create log directory on VPS
mkdir -p /var/log/loantrack
touch /var/log/loantrack/npa-cron.log
touch /var/log/loantrack/penalty-cron.log

# Rotate logs weekly (add to /etc/logrotate.d/loantrack)
/var/log/loantrack/*.log {
  weekly
  rotate 52
  compress
  missingok
...
*Document version 1.0 — LoanTrack NPA Classification Engine*  
