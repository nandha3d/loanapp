...
| Check | Status | Notes |
|---|---|---|
| `getActiveBranchId()` resolves for superadmin from cookie/header | ✅ | Reads `x-loantrack-active-branch` header first, then falls back to cookie |
...
| `active_branch_id` cookie forwarded as header | ✅ | `x-loantrack-active-branch` set in `nextWithTenantHeaders` |
